...
sys_exofork(void)
{
	...
	struct Env *newenv_store;
        int error_code;	
	if((error_code = env_alloc(&newenv_store, curenv->env_id)) < 0){
		return error_code;
	}
...
sys_env_set_status(envid_t envid, int status)
{
	...
	int error_code;
	if((error_code = envid2env(envid, &env_store, 1)) < 0){
		return error_code;	
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((uint32_t)va >= UTOP || PGOFF(va))
		return -E_INVAL;	
	if((perm & flag) != flag || (perm & ~PTE_SYSCALL))
		return -E_INVAL;	
	if(!(p = page_alloc(ALLOC_ZERO)))
		return -E_NO_MEM;
	if(page_insert(env_store->env_pgdir, p, va, perm) < 0){
		page_free(p);	
		return -E_NO_MEM;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct PageInfo *p;
	int flag = (PTE_U | PTE_P);
	if((error_code = envid2env(srcenvid, &srcenv_store, true)) < 0 || (error_code = envid2env(dstenvid, &dstenv_store, 1)) < 0)
		return error_code;
	if((uint32_t)srcva >= UTOP || (uint32_t)dstva >= UTOP || PGOFF(srcva) || PGOFF(dstva))
		return -E_INVAL;
	if(!(p = page_lookup(srcenv_store->env_pgdir, srcva, &pte_store)))	
		return -E_INVAL;
	if((perm & flag) != flag || (perm & ~PTE_SYSCALL))
		return -E_INVAL;
        if(!(*pte_store & PTE_W) && (perm & PTE_W))	
		return -E_INVAL;
	if(page_insert(dstenv_store->env_pgdir, p, dstva, perm) < 0)
		return -E_NO_MEM;
	return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	struct Env *env_store;
	int error_code;
	if((error_code = envid2env(envid, &env_store, true)) < 0)
		return error_code;
	if((uint32_t)va >= UTOP || PGOFF(va))
		return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	pte_t *pte_store;
	struct PageInfo *p;
	if((err_code = envid2env(envid, &dst_env, 0)) < 0)
		return err_code;
	if(!(dst_env->env_ipc_recving))
		return -E_IPC_NOT_RECV;
	if((uint32_t)srcva >= UTOP || (uint32_t)(dst_env->env_ipc_dstva) >= UTOP){		
		dst_env-> env_ipc_perm = 0;	
	}else{
		if(PGOFF(srcva) || (perm & flag) != flag || (perm & ~PTE_SYSCALL))
			return -E_INVAL;
		if(!(p = page_lookup(curenv->env_pgdir, srcva, &pte_store)))
			return -E_INVAL;
		if((perm & PTE_W) && !(*pte_store & PTE_W))
			return -E_INVAL;
		if(page_insert(dst_env->env_pgdir, p, dst_env->env_ipc_dstva, perm) < 0)
			return -E_NO_MEM;
...
sys_ipc_recv(void *dstva)
{
	...
	// modiding the status of the env calling this function makes this env cannot be scheduled.
	// representing block in logic by this way, but this function has returned in fact;
	return 0;
}
...
static int
sys_netpacket_send(void *addr, size_t len)
{
...
static int
sys_netpacket_recv(void *addr, size_t *len)
{
	user_mem_assert(curenv, addr, 0, PTE_U);
