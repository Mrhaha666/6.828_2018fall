...
openfile_alloc(struct OpenFile **o)
{
	...
	for (i = 0; i < MAXOPEN; i++) {
		...
		case 0: //not be mapped
			if ((r = sys_page_alloc(0, opentab[i].o_fd, PTE_P|PTE_U|PTE_W)) < 0)
				return r;
			/* fall through */
		case 1: // The only one ref is owned by fs_server itself and no other user's env is using this page, so we can overwrite this entry's content.
